...
describe('Читаемая запись результата', () => {
    ...
    /*
     * Вес не повторяется, пока не изменился, а подряд идущие одинаковые
     * подходы сворачиваются в кратность (§15). Строка нужна как ориентир
     * перед подходом, а не как опись: шесть одинаковых подряд читались
     * хуже, чем один раз с числом.
     */
    it('подходы прошлого раза сворачиваются в строку', () => {
        const sets = [
            s({ weight: 60, reps: 10 }),
            s({ weight: 60, reps: 9 }),
            s({ weight: 55, reps: 8 })
        ...
        equal(records.describeSession(sets, 'weight'), '60 кг × 10, 9, 55 × 8');
    });

    it('одинаковые подряд идут кратностью', () => {
        const шесть = Array.from({ length: 6 }, () => s({ weight: 30, reps: 40 }));

        equal(records.describeSession(шесть, 'weight'), '30 кг × 40 — 6 подходов');
    });

    /*
     * Свёртка только подряд идущих: «12, 10, 8» — это работа до отказа, и
     * общее «3 подхода» стёрло бы как раз то, ради чего на строку смотрят.
     */
    it('работа до отказа не сворачивается', () => {
        const sets = [s({ weight: 30, reps: 12 }), s({ weight: 30, reps: 10 }), s({ weight: 30, reps: 8 })];

        equal(records.describeSession(sets, 'weight'), '30 кг × 12, 10, 8');
    });
