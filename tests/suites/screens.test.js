...
describe('Экран: главная', () => {
    ...
    it('предлагает вернуться к просроченному составу', async () => {
        ...
        assert(has(view, '9 дн'), 'подпись — дни, а не множитель');
        ...
        assert(!has(view, 'забыто'), 'одни и те же названия дважды на экране не стоят');
    });
    ...
    it('предлагает упражнения, которым пора, когда состав не повторялся', async () => {
        ...
        assert(has(view, 'забыто'), 'выпавшее из очереди упражнение показано отдельной плашкой');
