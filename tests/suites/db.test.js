...
describe('Уборка давно удалённых записей', () => {
    ...
    it('давний день сворачивается в одно число', async () => {
        ...
        equal(живые[0].note, 'завтрак; обед; ужин',
            'заметки склеиваются: через год к дневному итогу один вопрос — «почему столько»');
    });
...
describe('Доставка новых базовых упражнений', () => {
    ...
    it('интервальные упражнения не требуют снаряда', async () => {
        // Именно интервальный блок, а не весь хвост списка: после него
        // дописаны домашний набор и гиря, а у гири снаряд как раз есть
        const свои = migrations.BASE_EXERCISES.slice(...migrations.INTERVAL_BLOCK);
        ...
        // Границы блока заданы числами, и без сверки они разъедутся с
        // содержимым при первой же вставке
        equal(свои[0].name, 'Прыжки «звёздочка»', 'блок начинается не там, где сказано');
        equal(свои[свои.length - 1].name, 'Приседания «сумо»', 'блок кончается не там, где сказано');
    });
});
