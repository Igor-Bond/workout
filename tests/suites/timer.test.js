...
/**
 * Ввод длительности во время идущего отсчёта (§16, Р-81).
 *
 * Человек вспоминает про паузу посреди неё и вводит её целиком — «сегодня же
 * десять минут», — а не остаток сверх прошедшего. Считать надо от начала
 * отдыха, и проверять это часами: сдвиг на разницу дал бы те же числа при
 * нулевом прошедшем времени и разошёлся бы только на настоящей паузе.
 */
describe('Вся длительность отдыха', () => {

    /** Часы под управлением: ждать настоящие полторы минуты в проверке нечего. */
    function часы(from = 1700000000000) {
        const было = Date.now;
        let сейчас = from;
        ...
        return {
            вперёд: (ms) => { сейчас += ms; },
            вернуть: () => { Date.now = было; }
        };
    }

    it('введённое считается всей паузой, а не остатком', () => {
        const clock = часы();
        ...
        try {
            ...
            equal(restTimer.retotal(120), true);
            equal(restTimer.remaining, 30,
                'две минуты от последнего подхода: полторы прошли, осталось полминуты');

            restTimer.retotal(600);
            equal(restTimer.remaining, 510,
                'второй ввод тоже считается от начала, а не от первого ввода');
        } finally {
            restTimer.stop();
            clock.вернуть();
        }
    ...
    /*
     * Прошедшее уже больше введённого — отдых кончился. Оборвать его молча
     * нельзя: человек ждёт звука, а не тишины, и «Пропустить» он не нажимал.
     */
    it('уже прошедшая пауза кончается сигналом, и ровно одним', async () => {
        const clock = часы();
        let финишей = 0;
        const off = restTimer.on('finish', () => { финишей += 1; });

        try {
            запустить(600);
            clock.вперёд(20000);

            equal(restTimer.retotal(10), true);
            equal(restTimer.running, false, 'десять секунд отдыха давно прошли');

            // Отсчёт проверяет часы четыре раза в секунду
    ...
    it('границы те же, что у всякой длительности', () => {
        const clock = часы();

        try {
            запустить(600);

            restTimer.retotal(99999);
            equal(restTimer.remaining, restTimer.LONGEST, 'опечатка не даёт часовой паузы');

            equal(restTimer.retotal('полторы минуты'), false, 'мусор не трогает отсчёт');
            equal(restTimer.remaining, restTimer.LONGEST);
        } finally {
            restTimer.stop();
            clock.вернуть();
        }
    ...
    it('законченному отдыху задавать нечего', () => {
        restTimer.stop();

        equal(restTimer.retotal(120), false, 'иначе ввод воскрешал бы законченный отдых');
        equal(restTimer.running, false);
    });
});
