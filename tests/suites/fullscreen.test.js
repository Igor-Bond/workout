...
/**
 * Из полного экрана должен быть выход (§31).
 *
 * Одно нажатие даёт два повода войти — сторож на pointerdown и сама кнопка
 * на click, — и оба они соревновались с выходом. Человек, включивший режим
 * на компьютере, оставался в нём: выключатель звал exit() раньше, чем
 * завершался вход, и полный экран возвращался сам собой.
 */
describe('Выход из полного экрана', () => {

    it('второй вход поверх начатого не запрашивается', async () => {
        const s = stub();

        try {
            const оба = await Promise.all([fullscreen.enter(), fullscreen.enter()]);

            equal(оба, [true, true], 'оба вызова считают, что вошли');
            equal(s.calls.enter, 1, 'у браузера спрошено один раз');
        } finally {
            await fullscreen.exit();
            s.restore();
        }
    ...
    it('выход дожидается начатого входа', async () => {
        const s = stub();

        try {
            // Вход не ждём — ровно так и получается при одном нажатии:
            // сторож начал, обработчик кнопки уже выключает
            const вход = fullscreen.enter();
            await fullscreen.exit();
            await вход;
