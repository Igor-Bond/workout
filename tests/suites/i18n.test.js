...
describe('Даты по языкам', () => {
    ...
    /*
     * Сама дата цифрами одинакова: порядок «день, месяц, год» читается
     * везде, а американское «08.27.2026» дало бы двадцать седьмой месяц.
     */
    it('дата цифрами не меняется', () => {
        withLang('ru', () => equal(dates.formatDate(день), '27.08.2026'));
        withLang('en', () => equal(dates.formatDate(день), '27.08.2026'));
    });
});
