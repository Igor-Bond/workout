...
describe('Выбор языка', () => {
    ...
    it('подстановки работают в обоих языках', () => {
        withLang('ru', () => equal(t('Обычно раз в {n}', { n: '3 дня' }), 'Обычно раз в 3 дня'));
        withLang('en', () => equal(t('Обычно раз в {n}', { n: '3 days' }), 'Usually every 3 days'));
    });
});
...
describe('Словари без повторов', () => {

    /**
     * Ключ, объявленный дважды, побеждает последний — и это молчаливо.
     *
     * Проверка полноты такого не ловит: она смотрит, всё ли переведено, а не
     * сколько раз. Повтор же означает, что словарь поехал: два места правят
     * одну строку, и одно из них правит впустую.
     */
    for (const язык of ['en', 'de']) {
        it(`«${язык}» — каждый ключ объявлен один раз`, async () => {
            const ответ = await fetch(`../js/i18n/${язык}.js`);
            const текст = await ответ.text();

            const счёт = new Map();

            for (const строка of текст.split('\n')) {
                const m = строка.match(/^ {4}'(.*)': '.*',$/);
                if (!m) continue;

                счёт.set(m[1], (счёт.get(m[1]) || 0) + 1);
            }

            const повторы = [...счёт.entries()].filter(([, n]) => n > 1).map(([k]) => k);

            equal(повторы.length, 0, `объявлено дважды: ${повторы.join(' · ')}`);
        });
    }
