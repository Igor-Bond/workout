...
describe('Тепловая карта', () => {
     ...
     * девять пикселей, где оттенки неразличимы, а карта читалась как серое
     * пятно. Вся история остаётся доступной листанием.
     */
    it('покрывает квартал целыми неделями', () => {
        const days = stats.heatmap([], NOW);

        equal(days.length, stats.HEAT_WEEKS * 7, 'ровно столько недель, сколько объявлено');
        equal(days.length % 7, 0, 'столбцы карты — целые недели');
        equal(new Date(days[0].day).getDay(), 1, 'начинается с понедельника');
    });

    it('листание уводит окно в прошлое, не оставляя разрывов', () => {
        const сейчас = stats.heatmap([], NOW);
        const прошлое = stats.heatmap([], NOW, { offset: 1 });

        equal(прошлое.length, сейчас.length);

        const DAY = 86400000;
        equal(прошлое[прошлое.length - 1].day + DAY, сейчас[0].day,
            'между окнами не должно теряться ни дня');
    });

    it('отрицательное смещение в будущее не уводит', () => {
        equal(stats.heatmap([], NOW, { offset: -5 })[0].day, stats.heatmap([], NOW)[0].day);
    });

    it('окон столько, сколько покрывает история', () => {
        const DAY = 86400000;

        equal(stats.heatWindows([], NOW), 1, 'без истории листать некуда');
        equal(stats.heatWindows([entry(3)], NOW), 1, 'всё в нынешнем окне');

        // Тренировка почти год назад — это четыре квартала
        const давняя = [{ workout: { startedAt: NOW - 360 * DAY }, sets: 1, volume: 0, exerciseIds: [] }];
        equal(stats.heatWindows(давняя, NOW), 4);
    });
