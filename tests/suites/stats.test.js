...
describe('Вес тела', () => {
    ...
    /*
     * Талия лежит в тех же записях, но мерят её реже веса (Р-88). Ряд по ней
     * обязан пропускать записи без замера: ноль в графике читался бы как
     * обхват в ноль сантиметров, а не как «в тот день не мерили».
     */
    it('ряд талии строится только по тем дням, когда мерили', () => {
        const записи = [
            { at: NOW - 20 * DAY, weight: 93, waist: 99 },
            { at: NOW - 10 * DAY, weight: 92 },
            { at: NOW, weight: 92, waist: 97 }
        ];

        const ряд = stats.bodySeries(записи, null, 'waist');

        equal(ряд.length, 2);
        equal(stats.bodyChange(ряд).delta, -2);
        equal(stats.bodySeries(записи, null).length, 3, 'вес при этом есть у всех');
    });
});
...
describe('Нагрузка с весом тела', () => {
    ...
    /*
     * Довес сюда не входит (Р-49): он считается тоннажем отдельно. Пока
     * прибавлялся и здесь, пояс на подтягиваниях попадал в счёт дважды — в
     * «Тоннаж» и внутрь «Со своим весом», в двух соседних плитках.
     */
    it('довес в нагрузку собственным весом не входит', () => {
        equal(stats.load({ reps: 8, weight: 10 }, 'reps', 78), 624, '8 × 78, и ни килограммом больше');
    });

    it('доля упражнения уменьшает нагрузку', () => {
        equal(stats.load({ reps: 10 }, 'reps', 80, 0.65), 520, 'отжимания — не весь вес тела');
    });
...
describe('Тепловая карта', () => {
    ...
    /*
     * Год снизу: пустые недели до первой тренировки — тоже сведения, по ним
     * видно, с какого месяца всё началось. Обрезать карту по длине истории
     * значит лишать её масштаба.
     */
    it('короче года не бывает', () => {
        equal(stats.HEAT_WEEKS, 52);
        equal(stats.heatWeeks([], NOW), 52, 'без истории — всё равно год');
        equal(stats.heatWeeks([давно(5)], NOW), 52);
        equal(stats.heatWeeks([давно(200)], NOW), 52, 'история в семь месяцев года не укорачивает');
    });

    it('история длиннее года растягивает охват', () => {
        const охват = stats.heatWeeks([давно(500)], NOW);

        assert(охват > 52, `500 дней — это больше года, вышло ${охват}`);
        assert(охват <= stats.HEAT_MAX_WEEKS);

        equal(stats.heatWeeks([давно(500), давно(3)], NOW), охват, 'считается по самой ранней');
    });
    ...
    it('охват доходит до карты', () => {
        equal(stats.heatmap([давно(200)], NOW).length, 52 * 7);
...
/*
 * Нагрузка собственным весом за период (§15.2, Р-49).
 *
 * До неё тренировка целиком на своём весе показывала в статистике ноль, и
 * ради этого ноля люди вписывали вес руками в поле, которое значит довес.
 */
describe('Объём собственным весом', () => {

    const NOW = Date.parse('2026-09-01T12:00:00');
    const DAY = 86400000;

    const упражнения = {
        отжимания: { id: 'отжимания', kind: 'reps', group: 'Грудь' },
        жим: { id: 'жим', kind: 'weight', group: 'Грудь' }
    };

    const подход = (exerciseId, over = {}) => ({
        exerciseId, reps: 10, performedAt: NOW - DAY, ...over
    });

    const вес = [{ at: NOW - 10 * DAY, weight: 80 }];

    it('считается по доле упражнения и весу тела на дату', () => {
        equal(stats.bodyVolume([подход('отжимания')], упражнения, вес, null, () => 0.65), 520);
    });

    it('упражнение со снарядом сюда не попадает', () => {
        equal(stats.bodyVolume([подход('жим', { weight: 60 })], упражнения, вес, null, () => 0.65), 0,
            'его нагрузка названа числом и лежит в тоннаже');
    });

    it('без веса тела не считается', () => {
        equal(stats.bodyVolume([подход('отжимания')], упражнения, [], null, () => 0.65), 0);
    });

    it('довес не удваивается', () => {
        const свой = stats.bodyVolume([подход('отжимания', { weight: 45 })], упражнения, вес, null, () => 0.65);

        equal(свой, 520, 'вписанные 45 кг — тоннаж, а не доля собственного веса');
    });

    it('удалённые подходы не считаются', () => {
        equal(stats.bodyVolume([подход('отжимания', { deletedAt: NOW })], упражнения, вес, null, () => 0.65), 0);
    });

    it('свой вес доходит до объёма по группам мышц', () => {
        const без = stats.muscleVolume([подход('отжимания')], упражнения, null);
        const с = stats.muscleVolume([подход('отжимания')], упражнения, null, { weights: вес, shareOf: () => 0.65 });

        equal(без[0].volume, 0, 'раньше грудь от отжиманий выглядела ненагруженной');
...
/**
 * Подходы по группам, разложенные по неделям (§66, Р-138).
 *
 * Ориентир недельного объёма — единственная мерка этой величины, и считается
 * она по полным неделям: нынешняя ещё идёт, и в понедельник утром её столбец
 * равен нулю.
 */
describe('Недельный объём по группам', () => {

    const WEEK = 7 * DAY;
    const понедельник = stats.weekStart(NOW);

    /** Подход на упражнение из группы, за n полных недель до нынешней. */
    const подход = (группа, недельНазад, сдвиг = 0) => ({
        exerciseId: группа,
        performedAt: понедельник - недельНазад * WEEK + сдвиг + 3600000
    });

    const упражнения = {
        Грудь: { id: 'Грудь', group: 'Грудь' },
        Ноги: { id: 'Ноги', group: 'Ноги' }
    };

    it('нынешняя неделя не считается: она ещё идёт', () => {
        const sets = [
            подход('Грудь', 0),   // эта неделя, ещё не дожита
            подход('Грудь', 1),
            подход('Грудь', 1)
        ];

        const [грудь] = stats.groupWeeks(sets, упражнения, { now: NOW, weeks: 4, tail: 1 });

        equal(грудь.sets, 2, 'подход нынешней недели в счёт не идёт');
        equal(грудь.perWeek, 2, 'и среднее считается по прошлой, а не по полутора');
    });

    /*
     * Неделя без единого подхода — это данные, а не пропуск: ради неё на
     * график и смотрят.
     */
    it('пустые недели остаются точками ряда', () => {
        const sets = [подход('Ноги', 4), подход('Ноги', 1)];

        const [ноги] = stats.groupWeeks(sets, упражнения, { now: NOW, weeks: 6 });

        equal(ноги.points.length, 6, 'ряд ровно по числу недель');
        equal(ноги.points.filter((p) => p.value === 0).length, 4, 'четыре недели человек ног не касался');
    });

    it('среднее берётся по четырём последним полным неделям', () => {
        const sets = [
            ...Array.from({ length: 12 }, () => подход('Грудь', 8)),   // давно и много
            подход('Грудь', 3), подход('Грудь', 3),
            подход('Грудь', 1)
        ];

        const [грудь] = stats.groupWeeks(sets, упражнения, { now: NOW, weeks: 12 });

        equal(грудь.perWeek, 0.8, 'три подхода за четыре недели — 0,8, а не 15 из давнего запоя');
    });

    /*
     * Нынешняя неделя в средний объём не идёт, но группа из-за неё не
     * пропадает: начатую в понедельник иначе не видно вовсе (Р-146).
     */
    it('группа, начатая на этой неделе, остаётся с отдельным счётом', () => {
        const sets = [
            { exerciseId: 'Ноги', performedAt: понедельник + 3600000 },
            { exerciseId: 'Ноги', performedAt: понедельник + 7200000 }
        ];

        const [ноги] = stats.groupWeeks(sets, упражнения, { now: NOW, weeks: 8 });

        equal(ноги.group, 'Ноги', 'группа обязана остаться на виду');
        equal(ноги.sets, 0, 'в полных неделях её ещё нет');
        equal(ноги.thisWeek, 2, 'а на этой неделе — два подхода');
    });

    it('группы идут от самой нагруженной к самой забытой', () => {
        const sets = [
            подход('Ноги', 1),
            подход('Грудь', 1), подход('Грудь', 1), подход('Грудь', 2)
        ];

        const итог = stats.groupWeeks(sets, упражнения, { now: NOW, weeks: 8 });

        equal(итог[0].group, 'Грудь', 'первой — та, на которую ходит больше всего');
    });
});

/**
 * Полоса «как у вас обычно» (§66, Р-139).
 *
 * Взята середина ряда, а не среднее с отклонением: её не сносит одна
 * бессонная ночь, и объяснить её можно одной фразой.
 */
describe('Обычная полоса величины', () => {

    it('это середина ряда — от четверти до трёх четвертей', () => {
        const полоса = stats.middleHalf([1, 2, 3, 4, 5]);
    ...
    /*
     * Ради этого середина и взята: одна бессонная ночь не должна расширять
     * полосу так, чтобы внутрь попало всё подряд.
     */
    it('одинокий выброс полосу не раздвигает', () => {
        const ровный = stats.middleHalf([50, 51, 52, 53, 54, 55]);
        const свыбросом = stats.middleHalf([50, 51, 52, 53, 54, 200]);

        equal(свыбросом.from, ровный.from, 'низ не сдвинулся');
        assert(свыбросом.to - свыбросом.from < 10, 'и полоса не растянулась на выброс');
    });

    it('меньше четырёх точек — полосы нет', () => {
        equal(stats.middleHalf([5, 6, 7]), null, 'у трёх чисел обычности не бывает');
        equal(stats.middleHalf([]), null);
    });

    it('ряд без разброса полосы не даёт', () => {
        equal(stats.middleHalf([7, 7, 7, 7, 7]), null, 'полоса нулевой высоты — это линия');
    });
});
