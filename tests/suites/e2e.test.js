...
import { тик } from '../helpers/dom.js';
...
/**
 * Чистая база — и обмен, доигравший до конца.
 *
 * Обмен запускается не только проверкой: его дёргает подписка на завершение
 * тренировки, и `sync.now()` обещания не ждёт — по замыслу, вызвавшему оно
 * нужно только чтобы перерисовать экран. Поэтому предыдущий сценарий вполне
 * может уйти, оставив `inProgress` поднятым, а следующий получит на первом же
 * `sync.now()` пустоту: обмен «уже идёт».
 *
 * Ловилось это через раз и зависело от того, насколько занята машина, — то
 * есть от чего угодно, кроме проверяемого. Ждём покоя здесь, один раз на все
 * сценарии: проверка обязана начинаться с известного состояния.
 */
async function clean() {
    await дождаться(() => !sync.inProgress);
 ...
 * Ждать условия.
 *
 * Опрос идёт оборотами очереди, а не по часам (Р-201). В свёрнутой вкладке
 * браузер растягивает таймеры до секунды, и опрос «раз в пять миллисекунд с
 * пределом в пять секунд» превращался в пять попыток вместо тысячи: обмен не
 * успевал доиграть, `clean()` возвращалась при поднятом `inProgress`, и
 * следующий сценарий получал стёртую посреди работы базу. Отсюда и брались
 * сорок провалов на чистом коде.
 *
 * Число оборотов щедрое: проверки гоняются в живом браузере, где вкладка
 * делит время с отрисовкой экранов и со звуковым движком. Меньшее ловило бы
 * не ошибку обмена, а занятость машины.
 */
async function дождаться(условие, оборотов = 2000) {
    for (let i = 0; i < оборотов; i++) {
...
describe('Сквозной путь: обмен с облаком', () => {
    ...
    it('законченная тренировка уезжает сама, без ухода со страницы', async () => {
        ...
        try {
            ...
            /*
             * Документ появился — но обмен ещё доигрывает: пишет метку
             * последней отправки и опускает `inProgress`. Отсоединять
             * поддельное облако на этом месте значило бы выдёргивать стол
             * из-под пишущего.
             */
            await дождаться(() => !sync.inProgress);
    ...
    it('частые поводы придерживаются, важный проходит всегда', async () => {
        ...
        try {
            ...
            /*
             * Ответ спрашивается у самого обмена, а не у часов (Р-146).
             *
             * Здесь стояло ожидание до пяти секунд: обмен запускали и ждали,
             * пока в поддельном облаке вырастет счётчик чтений. Под полным
             * набором проверка изредка не дожидалась и падала — одна из
             * девяти прогонов. Падающая через раз проверка хуже
             * отсутствующей: на неё перестают смотреть.
             *
             * Ждать нечего: sync.now отдаёт обещание, а придержанный обмен
             * возвращает пусто прямо на месте.
             */
            equal(sync.now({ notSooner: 60000 }), null, 'только что обменивались — второй раз незачем');
            equal(cloud.state.reads, было, 'и в облако никто не ходил');
    ...
    /*
     * Таблица, появившаяся позже устройства (§68, Р-160).
     *
     * Граница приёма одна на все таблицы и двигается по самому свежему, что
     * устройство видело. Появляется новая таблица — а граница на уже
     * работавшем устройстве стоит дальше, чем отметки у её записей, и обычный
     * отбор «новее границы» не находит в ней ничего. Не «пока не появятся
     * новые», а навсегда для всего, что легло в облако до обновления.
     */
    it('таблица, о которой устройство не знало, читается целиком', async () => {
        await clean();
        const cloud = fakeCloud();
        const restore = connect(cloud);
        ...
        try {
            const t = Date.now();
            const давняя = dbService.newId();

            // Запись легла в облако до обновления: отметка сервера у неё ранняя
            cloud.put('intake', {
                id: давняя, at: t - 86400000, kcal: 1900, note: 'вчера',
                updatedAt: t, syncedAt: 5
            });

            /*
             * Устройство, которое обменивалось до появления съеденного:
             * граница ушла далеко вперёд, а про таблицу оно ещё не слышало.
             */
            config.set('syncCursor', 900_000);
            config.set('syncStamped', true);
            config.set('syncPulled', ['exercises', 'templates', 'workouts', 'bodyWeight']);

            await sync.run({ silent: true });

            const дома = await dbService.listIntake();

            equal(дома.length, 1, 'давняя запись обязана доехать, хотя её отметка старше границы');
            equal(дома[0].kcal, 1900);

            // Второй раз таблицу целиком не перечитываем: она уже читана
            const было = cloud.state.reads;
            await sync.run({ silent: true });

            assert(cloud.state.reads - было < 4,
                `полный проход делается один раз, а не каждый обмен: прочитано ${cloud.state.reads - было}`);
        } finally {
            restore();
        }
    });
