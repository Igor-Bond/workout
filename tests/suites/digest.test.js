 ...
 *
 * Дефицит опирается на расход, а тот — на коэффициент активности с грубыми
 * ступенями. Приложение красит съеденное по такому потолку и обязано само
 * сказать, когда его оценка не сходится с действительностью.
 */
describe('Потолок против весов', () => {

    const внутри = { average: 2100, ceiling: 2394 };

    it('месяц в потолке при стоящем весе — числа не сходятся', () => {
        const строки = digest.condition({
            intake: внутри,
            body: { weight: { delta: 0.2, state: health.СПРАВКА } }
        });

        const сказано = строки.map((с) => с.text).join(' ');

        assert(сказано.includes('вес стоит'), сказано);
        assert(сказано.includes('расход посчитан выше') && сказано.includes('записано не всё'),
            `оба объяснения названы, выбирать за человека приложение не берётся: ${сказано}`);
    });

    it('вес пошёл — проверять нечего', () => {
        const строки = digest.condition({
            intake: внутри,
            body: { weight: { delta: -1.5, state: health.ХОРОШО } }
        });

        assert(!строки.some((с) => с.text.includes('вес стоит')),
            'расход сходится с весами — и молчим');
    });

    it('перебрали потолок — это другая новость', () => {
        const строки = digest.condition({
            intake: { average: 2600, ceiling: 2394 },
            body: { weight: { delta: 0.1, state: health.СПРАВКА } }
        });

        assert(!строки.some((с) => с.text.includes('вес стоит')),
            'вес стоит потому, что ели сверх потолка, — и это видно без нас');
    });

    it('без замеров веса проверки нет', () => {
        equal(digest.condition({ intake: внутри }).length, 1, 'останется только строка питания');
    });
});
