...
describe('Сигналы', () => {
    ...
    it('конец работы и круга идут вниз', () => {
        const стоп = beeper.VOICES.rest.map((p) => p.freq);
        const круг = beeper.VOICES.round.map((p) => p.freq);

        assert(стоп[1] < стоп[0], 'конец работы — вниз');
        assert(круг.every((f, i) => i === 0 || f < круг[i - 1]), 'конец круга — вниз ступенями');
    });

    /*
     * Раньше старт и конец программы были восходящими наборами нот и потому
     * путались. Различаться должно устройство сигнала: старт одиночный и
     * тянущийся, конец — ступени, и он единственный длиннее секунды.
     */
    it('старт и конец программы устроены по-разному', () => {
        ...
        assert(конец.every((f, i) => i === 0 || f.freq > конец[i - 1].freq), 'конец — вверх');
