...
describe('Сигналы', () => {
     ...
     * и то же «пик».
     */
    it('начало работы — подъём, конец работы — спад', () => {
        const го = beeper.VOICES.go[0];
        const стоп = beeper.VOICES.rest.map((p) => p.freq);

        assert(го.to > го.freq, 'начало работы — сплошной взлёт тона');
        assert(стоп[1] < стоп[0], 'конец работы — вниз');
    });

    /*
     * Раньше и старт, и конец программы были восходящими наборами нот и
     * потому путались между собой. Различаться должно само устройство
     * сигнала, а не только высота: там сплошная линия, здесь ступени.
     */
    it('старт и конец программы устроены по-разному', () => {
        const го = beeper.VOICES.go;
        const конец = beeper.VOICES.done;

        equal(го.length, 1, 'старт — одна сплошная линия');
        assert(конец.length >= 3, 'конец — отдельные ступени');
        assert(!конец.some((p) => p.to), 'у конца программы подъёма тона нет');
