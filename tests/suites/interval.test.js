...
describe('Сигналы', () => {
    ...
    /*
     * Конец программы читается как фраза, а не как набор ударов.
     *
     * Двойной удар на вершине здесь был и ритма не дал: пятый удар через
     * короткую паузу слышался запинкой. Ровный шаг и долгая нота в конце —
     * раз-два-три и всё.
     */
    it('конец программы идёт ровным шагом', () => {
        const шаги = [...new Set(beeper.VOICES.done.map((p) => p.after || 0))].sort((a, b) => a - b);
        const между = шаги.slice(1).map((s, i) => +(s - шаги[i]).toFixed(3));

        assert(между.length >= 3, 'три шага и вершина');
        assert(Math.max(...между) - Math.min(...между) < 0.02,
            `шаги разной длины (${между.join(', ')}) — в такой короткой фигуре это слышно как сбой`);
    });

    /*
     * Вершина держится, но недолго: тон около килогерца телефонный динамик
     * тянет плохо, и звенеть после неё остаётся тоника октавой ниже.
     */
    it('вершина последняя и самая долгая из ярких', () => {
        const верх = Math.max(...beeper.VOICES.done.map((p) => p.freq));
        const яркие = beeper.VOICES.done.filter((p) => p.freq === верх);

        equal(яркие.length, 1, 'на вершине один удар, а не два');
        assert(яркие[0].length >= 0.8, `вершина ${яркие[0].length} с — слишком коротко для конца`);
        assert(яркие[0].length <= 1.2, `вершина ${яркие[0].length} с — динамик захрипит`);

        const позже = beeper.VOICES.done.filter((p) => (p.after || 0) > (яркие[0].after || 0));
        equal(позже.length, 0, 'после вершины ударов нет');
    });
    ...
    it('во время работы идёт тихий пульс', () => {
        ...
        equal(пульс, [10, 13, 16, 19], 'каждые три секунды, кроме последних трёх');
    });

    /*
     * Шаг отмеряется от конца отрезка, а не от начала.
     *
     * Считая от начала, последний пульс попадал куда придётся: в
     * двадцатисекундной работе он звучал за две секунды до отсчёта, и на слух
     * выходило «тук… тук-тук-тук» — сбой вместо ритма. От конца расстояние до
     * отсчёта всегда одно и то же, какой бы длины ни была работа.
     */
    it('от последнего пульса до отсчёта всегда один шаг', () => {
        for (const work of [10, 13, 20, 21, 30, 45]) {
            const свои = interval.cues(interval.build({ work, rest: 10, rounds: 1, lead: 0 }, УПР));

            const пульс = свои.filter((c) => c.type === 'pulse' && c.at < work).map((c) => c.at);
            if (пульс.length === 0) continue;

            const последний = Math.max(...пульс);
            const отсчёт = Math.min(...свои.filter((c) => c.type === 'count' && c.at < work).map((c) => c.at));

            equal(отсчёт - последний, 3, `работа ${work} с: между пульсом и отсчётом ${отсчёт - последний} с`);
        }
    });
