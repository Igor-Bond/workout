...
const EN = {
    ...
    howTo: {
        ...
        'Махи с резинкой в стороны': 'Stand on the band, arms by the sides. Raise the arms to shoulder line without shrugging. Bending the elbows shortens the lever and buys more reps.',
...
const DE = {
    ...
    howTo: {
        ...
        'Махи с резинкой в стороны': 'Auf das Band stellen, Arme seitlich am Körper. Die Arme bis auf Schulterhöhe heben, Schultern nicht hochziehen. Gebeugte Ellbogen verkürzen den Hebel — so gelingen mehr Wiederholungen.',
...
/**
 * Названия групп, какие приложение ставит при первом запуске на этом языке.
 *
 * Наружу — ради проверки (Р-147): те же названия переводятся ещё раз, уже
 * словарём интерфейса, когда группа приходит с чужого запуска. Двум спискам
 * разъезжаться нельзя, а разъехались они сразу: «Пресс» встал как «Abs» в
 * поставке и как «Core» в словаре.
 */
export function deliveredGroups(lang) {
    return ALL[lang]?.groups || {};
}
