        ...
        },
        {
            ...
            blocks: [
                ...
                { rows: [
                    ...
                    ['Следом', 'Плашки под карточкой — остальная очередь. Подпись говорит, насколько состав опаздывает: «+3 дн» — просрочен на столько, «−4 дн» — столько ещё в запасе, «0 дн» — ровно сегодня. Промежуток берётся из вашей же истории за последние 12 недель и пересчитывается сам: станете делать реже — порог поднимется. Но прежде опоздания смотрятся мышцы: состав, чья группа работала недавно, уходит вниз, как бы он ни опаздывал. Тренировки типа «Зарядка» в очереди не участвуют — их делают каждое утро и без напоминания.'],
                    ['Забытое', 'Плашка пунктиром во всю ширину, своим разделом. Упражнение, которое не делалось дольше обычного для него срока и не входит ни в один повторяющийся состав. Стрелки по краям листают такие упражнения, подпись говорит, сколько дней прошло с прошлого раза, а нажатие на название собирает из них тренировку. Порядок — от давнего к недавнему; то, чего не было дольше 12 недель, из списка выпадает: это уже не забытое, а оставленное.'],
        ...
        },
        {
            ...
            blocks: [
                ...
                { rows: [
                    ...
                    ['Then', 'The chips under the card are the rest of the queue. The label says how late a line-up is running: “+3 d” means overdue by that much, “−4 d” means that much still in hand, “0 d” means exactly today. The gap comes from your own history over the last 12 weeks and recomputes itself: do something less often and the threshold rises. But muscles come before lateness: a line-up whose group worked recently drops down the queue however overdue it is. Workouts of the “Morning routine” type stay out of the queue — you do them every morning without being reminded.'],
                    ['Forgotten', 'A dashed full-width chip in a section of its own. An exercise that has gone longer than its usual gap and belongs to no repeating line-up. The arrows on either side flip through them, the label says how many days have passed since last time, and a tap on the name builds a workout out of them. Order runs from the longest gone to the most recent; anything missing for over 12 weeks drops off the list — that is no longer forgotten but given up.'],
        ...
        },
        {
            ...
            blocks: [
                ...
                { rows: [
                    ...
                    ['Danach', 'Die Chips unter der Karte sind der Rest der Warteschlange. Die Beschriftung sagt, wie sehr eine Zusammenstellung überfällig ist: „+3 T“ heißt so viel überfällig, „−4 T“ heißt so viel noch übrig, „0 T“ heißt genau heute. Der Abstand stammt aus deinem eigenen Verlauf der letzten 12 Wochen und rechnet sich selbst neu: Machst du etwas seltener, steigt die Schwelle. Doch vor der Überfälligkeit zählen die Muskeln: Eine Zusammenstellung, deren Gruppe kürzlich gearbeitet hat, rutscht nach unten, so überfällig sie auch sein mag. Trainings vom Typ „Morgengymnastik“ stehen nicht in der Warteschlange — die machst du jeden Morgen ohne Erinnerung.'],
                    ['Vergessen', 'Ein gestrichelter Chip über die ganze Breite, in einem eigenen Abschnitt. Eine Übung, die länger als sonst nicht dran war und zu keiner wiederkehrenden Zusammenstellung gehört. Die Pfeile an den Seiten blättern durch sie, die Beschriftung sagt, wie viele Tage seit dem letzten Mal vergangen sind, und ein Tippen auf den Namen baut daraus ein Training. Sortiert wird vom Längsten zum Kürzesten; was länger als 12 Wochen fehlt, fällt heraus — das ist nicht mehr vergessen, sondern aufgegeben.'],
