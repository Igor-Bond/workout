...
const VOICES = {
     ...
     */
    go: [{ kind: BELL, freq: 1046, length: 1.3, gain: 0.85, send: 0.4 }],
    ...
    // Конец круга — три удара вниз: пауза здесь длинная, и это слышно
    round: [
        { kind: BELL, freq: 784, length: 0.45, gain: 0.7, send: 0.28 },
        { kind: BELL, freq: 659, length: 0.45, gain: 0.7, send: 0.28, after: 0.17 },
        { kind: BELL, freq: 440, length: 1.4, gain: 0.75, send: 0.4, after: 0.34 }
    ],

    /*
     * Конец программы — восходящее трезвучие с долгой последней нотой.
     *
     * Оно одно длится больше двух секунд и одно разрешается вверх: и то и
     * другое само по себе говорит, что кончилось всё, а не очередной
     * отрезок.
     */
    done: [
        { kind: BELL, freq: 523,  length: 0.35, gain: 0.75, send: 0.25 },
        { kind: BELL, freq: 659,  length: 0.35, gain: 0.78, send: 0.25, after: 0.14 },
        { kind: BELL, freq: 784,  length: 0.4,  gain: 0.8,  send: 0.3,  after: 0.28 },
        { kind: BELL, freq: 1046, length: 2.2,  gain: 0.9,  send: 0.5,  after: 0.42 }
    ]
};
