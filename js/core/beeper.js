 ...
 * до 0,85. Остаток до единицы не запас на всякий случай, а место для
 * наложений: в конце программы несколько нот звучат разом, а подъём стоит
 * после сжатия и суммы уже не придерживает.
 *
 * Владелец сказал, что звук приходится выкручивать на максимум, и замер это
 * подтвердил: запаса по пику оставалось всего процентов пятнадцать, то есть
 * одним подъёмом громче было не сделать. Сжатие ужесточено — порог с −30 до
 * −40, отношение с 12 до 20, — и подъём поднят с 1,75 до 2,9. Средний
 * уровень вырос на пятую часть у громких сигналов и почти вдвое у отсчёта,
 * который был тише всех и терялся в зале.
 */
const MAKEUP = 2.9;
...
function context() {
     ...
     * Удар при этом остаётся ударом: отношение пика к среднему держится
     * около шести, а «дзинь» превращается в «шшш» гораздо ниже — там, где
     * оно подходит к двум.
     */
