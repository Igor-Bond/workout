...
const VOICES = {
    ...
    /*
     * Начало работы — один долгий ровный тон.
     *
     * Три щелчка отсчёта и длинный сигнал — это стартовая команда, как на
     * дорожке: узнаётся без объяснений и звучит серьёзно.
     *
     * Скольжение тона вверх пробовалось и отвергнуто: непрерывный подъём —
     * это звук из мультфильма, а не команда начинать.
     *
     * Среди остальных он единственный одиночный и тянущийся: отсчёт короткий,
     * конец работы и круга — нисходящие пары и тройки, конец программы —
     * четыре ступени. Спутать не с чем.
     */
    go: [{ freq: 1046, length: 0.55, gain: 0.9 }],
