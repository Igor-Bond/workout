...
const VOICES = {
    ...
    /*
     * Конец программы — три коротких шага вверх и долгий на вершине.
     *
     * Двойного удара на вершине здесь был: «та-да». Ритма он не давал —
     * пятый удар через короткую паузу слышался запинкой, а не фигурой.
     * Ровный шаг и одна долгая нота в конце читаются как фраза: раз-два-три
     * и всё.
     *
     * Шаги одинаковые, до сотой доли: неровность в такой короткой фигуре
     * слышна сразу и звучит как сбой, а не как замысел.
     *
     * Верхняя нота держится секунду, не дольше. Длинный тон около килогерца
     * телефонному динамику не по силам — он отзывается хрипом. Звенеть же
     * после неё остаётся не она, а тоника октавой ниже с ещё одной октавой
     * под ней: вес даёт низ, а не высота.
     */
    done: [
        // Разбег ровным шагом: до — ми — соль
        { kind: BELL, freq: 523,  length: 0.3,  gain: 0.72, send: 0.2 },
        { kind: BELL, freq: 659,  length: 0.3,  gain: 0.74, send: 0.2,  after: 0.13 },
        { kind: BELL, freq: 784,  length: 0.3,  gain: 0.78, send: 0.22, after: 0.26 },

        // Вершина: тот же шаг, но нота держится
        { kind: BELL, freq: 1046, length: 1,    gain: 0.95, send: 0.34, after: 0.39 },

        // Под ней — тоника и октава ниже: они и звенят до конца
        { kind: BELL, freq: 523,  length: 2.2,  gain: 0.85, send: 0.44, after: 0.39 },
        { kind: BELL, freq: 262,  length: 2.4,  gain: 0.42, send: 0.24, after: 0.39 }
    ]
};
