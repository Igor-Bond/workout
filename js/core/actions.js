...
export const actions = {
    ...
    /**
     * Имена, на которые кто-то подписан, — для проверки (Р-149).
     *
     * Кнопка с именем, которого никто не слушает, нажимается и молчит: с виду
     * живая, а не делает ничего. Такие уже находились глазами (Р-115), и
     * найти их можно только так — сверив разметку с этим списком. Наружу
     * отдаётся копия: подписку меняют через `on`, а не правкой списка.
     */
    names() {
        return { click: [...clickHandlers.keys()], change: [...changeHandlers.keys()] };
    },
