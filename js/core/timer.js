 ...
 *
 * Одного усиления не хватило: пик упирается в потолок дорожки раньше, чем
 * средний уровень доходит до нужного. Поэтому тон идёт через то же сжатие с
 * подъёмом, что и интервальные сигналы, — оно придерживает всплеск в начале
 * тона, а подъём после него возвращает громкость.
 *
 * Замер после: средний уровень 0,112 против 0,115 у «начали», пик 0,85 при
 * потолке 0,95. Запас до единицы нужен: на нём начинается хрип.
 */
 ...
 * Подъём после сжатия.
 *
 * Число подобрано замером, а не на слух: со сжатием без подъёма сигнал
 * проседал до среднего уровня 0,045 — вдвое тише, чем был до всей затеи.
 */
const TONE_MAKEUP = 4;
...
/**
 * Выход со сжатием и подъёмом — тот же приём, что у интервальных сигналов.
 *
 * Одним усилением догнать их не вышло: пик упирается в потолок дорожки
 * раньше, чем средний уровень доходит до нужного. Сжатие придерживает
 * всплеск в начале тона, подъём после него возвращает громкость — и слышимая
 * громкость растёт при том же пике.
 */
let outlet = null;
...
function scheduleSignal() {
    ...
    try {
        ...
        TONES.forEach((frequency, i) => {
            ...
            osc.connect(gain).connect(outlet || a.destination);
            osc.start(at);
