 ...
 *
 * Три секунды, а не пять, и отсчитываются они от конца отрезка, а не от
 * начала. Считая от начала, последний пульс попадал куда придётся: в
 * двадцатисекундной работе он звучал за две секунды до отсчёта «три-два-
 * один», и на слух выходило «тук… тук-тук-тук» без всякого ритма. От конца
 * последний пульс всегда отстоит от отсчёта ровно на шаг, и вся работа
 * отбивается ровным темпом до самого счёта.
 */
const PULSE = 3;
...
/**
 * Повторное напоминание в длинной паузе: за сколько секунд до работы.
 *
 * Минута отдыха между кругами — это минута, за которую сказанное вначале
 * успевает забыться. Десять секунд — столько, чтобы успеть встать и занять
 * место, но не столько, чтобы напоминание слилось с отсчётом.
 */
export const REMIND = 10;

/**
 * Короче какой паузы напоминать не надо.
 *
 * Обычный отдых в табате — десять секунд, и второе напоминание в нём
 * наложилось бы на первое. Порог отделяет паузу, в которой есть что забыть,
 * от паузы, которая сама короче напоминания.
 */
const REMIND_FROM = 25;
...
export const interval = {
    ...
    /**
     * Стоит ли повторить название под конец этой паузы.
     *
     * Отдых между кругами — минута, и сказанное в его начале к концу
     * забывается: подходишь к работе, не помня, к какой именно. Повтор
     * звучит за REMIND секунд до неё — уже как команда готовиться, а не как
     * объявление.
     *
     * Условие то же, что и у первой фразы: упражнение должно смениться.
     * Иначе программа из одного упражнения с длинным отдыхом принялась бы
     * повторять его название каждый круг по два раза.
     */
    remindAt(phases = [], index = 0) {
        const phase = phases[index];
        if (!phase || phase.seconds < REMIND_FROM) return null;

        return interval.announceAt(phases, index);
    },
    ...
    cues(phases = []) {
        ...
        phases.forEach((phase, i) => {
             ...
             *
             * Шаг отмеряется от конца: последний пульс обязан отстоять от
             * отсчёта ровно на шаг, а не на сколько придётся. Первый при этом
             * не жмётся к началу работы — там только что ударил свой сигнал,
             * и пульс сразу за ним слился бы с ним.
             */
            if (phase.kind === 'work') {
                for (let left = COUNT + PULSE; left < phase.seconds; left += PULSE) {
                    const when = at + phase.seconds - left;
                    if (when >= at + PULSE) list.push({ at: when, type: 'pulse' });
                }
            }
