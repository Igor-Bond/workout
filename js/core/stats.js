...
export const stats = {
    ...
    /**
     * Середина ряда: от четверти до трёх четвертей (§66, Р-139).
     *
     * Полоса «как у вас обычно». Взята середина, а не среднее со
     * стандартным отклонением, по двум причинам: её не сносит одна
     * бессонная ночь, и объяснить её можно одной фразой — половина всех
     * замеров лежит внутри. Отклонение объяснимо только формулой, а
     * объяснение формулой на этом экране запрещено.
     *
     * Меньше четырёх точек — полосы нет: у трёх чисел «середина» — это
     * среднее из них, и никакой обычности она не описывает.
     */
    middleHalf(values = []) {
        const ряд = values.filter((v) => Number.isFinite(v)).sort((a, b) => a - b);

        if (ряд.length < 4) return null;

        const доля = (q) => {
            const место = (ряд.length - 1) * q;
            const низ = Math.floor(место);
            const верх = Math.ceil(место);

            return низ === верх ? ряд[низ] : ряд[низ] + (ряд[верх] - ряд[низ]) * (место - низ);
        };

        const from = доля(0.25);
        const to = доля(0.75);

        // Полоса нулевой высоты — не полоса, а линия: ряд стоит на месте
