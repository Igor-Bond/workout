...
export const format = {
    ...
    /**
     * Число как есть, только с местным разделителем (Р-145).
     *
     * Отличие от decimal: тот дополняет нулями до заданного знака, и «3»
     * превращается в «3,0», а «1,6» — в «1,60». Есть величины, у которых
     * лишний ноль не точность, а помеха: индекс массы тела, коэффициент
     * активности, подходы в неделю. Раньше в таких местах стояла запятая
     * прямо в коде — и по-английски «0,55» соседствовало с «92.6».
     */
    plain(value) {
        if (!Number.isFinite(Number(value))) return '—';
        return String(value).replace('.', format.point());
    },
