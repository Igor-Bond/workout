...
export const dialog = {
     ...
     */
    /**
     * Сменить строку в открытом окне ожидания (Р-192).
     *
     * Ожидание ожиданию рознь. Ответ из сети приходит целиком и сказать о нём
     * по дороге нечего, а разговор с весами идёт шагами — «ищу», «нашёл»,
     * «встаньте на весы», — и каждый шаг это ответ на вопрос «работает ли
     * вообще». Держать их полосой наверху, пока окно говорит одно и то же,
     * значило бы разложить один разговор по двум местам.
     *
     * Молча ничего не делает, если окна нет: шаг может прийти и после того,
     * как всё кончилось.
     */
    say(text) {
        const строка = document.querySelector('.dialog-backdrop .dialog-text');
        if (строка) строка.textContent = String(text || '');
    },

    /**
     * Закрыть то, что открыто сейчас (Р-192).
     *
     * Нужно окну ожидания. Все остальные закрываются сами — нажатием, — а
     * это ждёт не человека, а ответ снаружи, и закрыть его может только тот,
     * кто этого ответа дождался.
     *
     * Раньше обходились без: окно ожидания сменялось следующим окном, и
     * подмена закрывала прежнее. Но там, где за ожиданием окна нет — ключ
     * принят, забор с часов кончился, — затемнение осталось бы висеть.
     */
    close() {
