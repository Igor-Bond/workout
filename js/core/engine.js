...
export const engine = {
    ...
    /**
     * Закрылся ли круг этим подходом (§16.1, Р-170).
     *
     * Круг в обычной тренировке — это проход по всем упражнениям плана, и
     * кончается он там, где выбор возвращается назад по списку. По этому
     * возврату его и видно: следующим идёт упражнение, стоящее в плане выше
     * нынешнего, — значит список кончился и начался сначала.
     *
     * Строго выше, а не «не ниже». Когда незакрытым осталось одно
     * упражнение, выбор возвращается к нему же — и каждый подход считался бы
     * концом круга, хотя кругов там больше нет.
     *
     * Ничего не осталось — круга тоже нет: тренировка кончилась, и пауза
     * после неё никому не нужна.
     */
    closesRound(plan = [], sets = [], currentId = null) {
        const from = plan.findIndex((p) => p.exerciseId === currentId);
        if (from < 0) return false;

        const next = engine.nextCircuit(plan, sets, currentId);

        return !!next && next.planIndex < from;
    },
     ...
     *
     * своё — то, что известно об упражнении помимо плана: постоянный довес
     * резинки (Р-86). Приносится готовым, потому что справочника движок не
     * знает. Первым подходом он и подставляется, дальше ведёт запись.
     */
    prefill(plan = [], sets = [], exerciseId, своё = {}) {
        ...
        return {
            ...
            weight: item?.weight || своё?.weight || null,
