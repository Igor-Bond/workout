...
export const rhythm = {
    ...
    dueWorkouts(entries = [], now = Date.now(), { weeks = RECENT_WEEKS, limit = 4, cap = 3, groupOf = null, background = null, includeToday = false } = {}) {
        ...
        /*
         * Упражнения, сделанные сегодня (Р-190).
         *
         * Состав сравнивается целиком, ключом из всех его упражнений, и
         * сделанный сегодня из очереди уходит. Но «отжимания + складка» и
         * «отжимания + подтягивания» — два разных ключа: первый сегодня
         * закрыт, второй просрочен и встаёт наверх, и в его названии первыми
         * стоят те же отжимания, которые человек только что сделал.
         *
         * Соседний счёт — забытые упражнения — про сегодняшнее знает и такого
         * не допускает. Знать об этом должны оба.
         *
         * При `includeToday` счёт не ведётся вовсе: там сегодняшнее не повод
         * что-либо прятать — так показывается зарядка, у которой «сегодня уже
         * было» это и есть ответ.
         */
        const сегодняшние = new Set();

        if (!includeToday) {
            for (const entry of рабочие) {
                if (startOfDay(entry.workout.startedAt) !== today) continue;

                for (const id of entry.exerciseIds || []) сегодняшние.add(id);
            }
        }
        ...
        return [...groups.values()]
            .map((group) => {
                ...
                return {
                    ...
                    // Задевает ли состав то, что уже сделано сегодня (Р-190)
                    touchedToday: group.exerciseIds.some((id) => сегодняшние.has(id)),

                    /*
                     * Пора ли: срок вышел и мышцы отдохнули (Р-63).
                     *
                     * Единственный признак, который поднимает состав над
                     * рядом по давности. Требуется и надёжность промежутка:
                     * при одном наблюдении «пора» — это догадка, а не срок,
                     * и выдавать её за рекомендацию нельзя.
                     */
                    due: gaps.length >= 2 && interval !== null
                        && daysSince >= interval && отдохнул(group.exerciseIds)
                };
            })
            .filter((g) => g.interval && (includeToday || g.daysSince >= 1) && g.skipped <= cap)
            /*
             * Два ключа, и только два (Р-63).
             *
             * Первый — «пора»: срок состава вышел и мышцы отдохнули. Такой
             * поднимается над всеми, и именно он попадает в карточку. Без
             * этого ключа карточка звала к тому, до чего дольше всего не
             * доходили, даже когда срок ему ещё не вышел, — а тот, кому
             * ровно пора, стоял плашкой ниже. На истории владельца это
             * случилось буквально: в карточке отжимания, которым рано на два
             * дня, под ней бицепс, которому пора ровно сегодня.
             *
             * Второй — дни с прошлого раза, от большего к меньшему. Не
             ...
             * трижды был прав.
             *
             * Отдыха мышц отдельным ключом больше нет. Он входит в «пора» и
             * там делает свою работу — не звать к трицепсу на следующий день
             * после трицепса. А поверх ряда по давности он давал только
             * перестановки: «2 дн» вставало выше «4 дн», потому что у
             * первого мышца успела отдохнуть. Верно по существу и дико на
             * вид, а объяснить это подписью, которая говорит о днях, нечем.
             */
            /*
             * Задевающий сегодняшнее — в конец, и раньше всех прочих ключей
             * (Р-190).
             *
             * Раньше «пора» намеренно: карточка наверху значит «делай это
             * сейчас», и состав, треть которого человек сделал час назад, —
             * плохой ответ на «сейчас», каким бы просроченным он ни был. Из
             * очереди он при этом не пропадает: доделать начатое законно, и
             * решать это человеку.
             */
            .sort((a, b) => (a.touchedToday - b.touchedToday)
                || (b.due - a.due)
