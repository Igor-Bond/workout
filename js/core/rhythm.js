...
/**
 * Фоновая тренировка — та, что делается сама собой, без выбора (§29.1).
 *
 ...
 *
 * Признак — тип, а не частота. По частоте было бы «умнее»: ежедневное
 * распознавалось бы само, как бы его ни назвали. Но тогда состав вылетал бы
 * из очереди в тот день, когда человек стал делать его чаще, и возвращался,
 * когда стал реже, — без всякого его участия и без объяснения. Тип человек
 * ставит сам, и правило остаётся предсказуемым.
 *
 * Сравнивается и с ключом, и с подписью: тип хранится переведённым, как и
 * у интервального (§50), и «Morgengymnastik» — та же зарядка.
 *
 * Живёт здесь, а не на главном экране, потому что пользуются им двое: очередь
 * составов и ориентир прошлого раза на выполнении (Р-54). Разъехавшись, они
 * называли бы фоном разное.
 */
export const BACKGROUND = 'Зарядка';

export const isBackground = (workout) =>
    workout?.type === BACKGROUND || workout?.type === t(BACKGROUND);
...
/**
 * Сколько последних выполнений смотреть, ища чередование объёма (Р-58).
 *
 * Шесть — это два полных прохода даже для тройки и запас сверху. Больше
 * брать вредно: человек мог держать другое чередование полгода назад, и оно
 * не должно спорить с нынешним.
 */
const CYCLE_WINDOW = 6;

/** Самый длинный цикл, который приложение берётся распознать. */
const MAX_CYCLE = 3;
...
export const rhythm = {
    ...
    /**
     * Следующее значение повторяющегося ряда (§10.2, Р-58).
     *
     * Приложение подставляет числа «как в прошлый раз», и для чередования
     * это неверно ровно через раз: сделал бицепс на 6 подходов — оно
     * предложит 6 снова, хотя по кругу идёт 12.
     *
     * Здесь ряд проверяется на цикл длиной от одного до трёх. Единица — это
     * постоянная величина, и она тоже цикл: «всегда 6» предсказывается тем
     * же правилом, что и «6, 12». Тройка нужна, когда позиция крутит три
     * состояния — скажем, отжимания на 6, отжимания на 12 и отжимания
     * Тайсона.
     *
     * Берётся самый короткий подошедший цикл: ряд «6, 6, 6, 6» подходит и
     * под единицу, и под двойку, но говорить о чередовании там не о чем.
     *
     * Требуется два полных прохода — иначе это не цикл, а совпадение. Ряд
     * «6, 12» ничего не обещает: за ним с тем же правом идёт и 6, и что
     * угодно ещё.
     *
     * null означает «цикла не видно»: вызывающий подставляет прошлое
    ...
    cycle(values = [], { maxCycle = MAX_CYCLE, window = CYCLE_WINDOW } = {}) {
        ...
        for (let length = 1; length <= maxCycle; length++) {
            ...
            let сходится = true;
            for (let i = length; i < ряд.length; i++) {
    ...
    dueExercises(entries = [], now = Date.now(), { limit = null, skip = null, weeks = RECENT_WEEKS } = {}) {
        ...
        /*
         * Общий простой здесь НЕ вычитается — в отличие от очереди составов.
         *
         * Замысел был обратный: после перерыва просрочено сразу всё, человек
         * не забыл упражнения, а просто не тренировался, — значит перерыв
         * надо вычесть. Сделали и проверили: лечить нечего, а сломать можно.
         *
         * Лечить нечего потому, что список и так короток — он обрезается
         * размером обычной тренировки, тремя-пятью упражнениями. Раздуться
         * ему не во что.
         *
         * А сломать можно законный случай: человек вернулся после перерыва, и
         * ему как раз полезно напомнить, чем он занимался. С поправкой раздел
         * молчал бы ровно тогда, когда нужнее всего, — проверки это и
         * показали, отказавшись предлагать что-либо после недельной паузы.
         */
