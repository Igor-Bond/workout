...
/**
 * Что считается «сейчас» для очереди составов.
 *
 * Двенадцать недель. Было восемь — и этого не хватало редким тренировкам:
 * чтобы у состава был известен промежуток, нужно три занятия внутри окна,
 * то есть промежуток не больше половины окна. Восемь недель обрезали всё,
 * что реже раза в три недели, — а именно такое и забывают. Двенадцать
 * поднимают границу до шести недель и оставляют запас на пропуск.
 *
 * Дальше растягивать нельзя: прошлогодняя привычка не должна лезть в
 * быстрый старт, а промежуток, посчитанный по полугодию, описывает уже не
