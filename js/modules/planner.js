...
export const planner = {
    ...
    async render() {
        ...
        return ui.html`
            ...
            <div class="card">
                <div class="card-title">${t('Вставить план')}</div>
                <p class="hint">
                ...
                <textarea id="plan-text" class="report-text" rows="12"
                          placeholder="${t('С 06.09.2026, 8 недель')}">${черновик?.text || ''}</textarea>
...
actions.on('sheet-apply', async () => {
    ...
    await dialog.alert({
        ...
        text: t('Главный экран будет вести по нему. Пропущенное сдвигается внутри недели, а с понедельника сетка восстанавливается.')
    });
});
