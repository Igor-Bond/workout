...
function restOf(exerciseId, кругЗакрыт = false) {
    /*
     * Порядок старшинства:
     *
    ...
    /*
     * Пауза после круга (§16.1, Р-170).
     *
     * Стоит выше паузы упражнения намеренно: строка плана говорит об
     * упражнении, а эта — о переходе, и когда речь именно о переходе,
     * сказанное о нём точнее. Живая правка человека всё равно выше обеих:
     * она сказана только что и про сейчас.
     */
    if (кругЗакрыт) {
        const круг = restTimer.clamp(view?.workout?.roundRest);
        if (круг) return круг;
    }
...
async function записатьПодходНабело(values) {
    ...
    // тренировки, она действует и на следующие упражнения. Исключение —
    // конец круга: там своя пауза, названная при сборе (§16.1)
    restTimer.start(
        restOf(currentId, engine.closesRound(workout.plan, after, currentId)),
        currentId
    );
