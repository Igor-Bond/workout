...
/**
 * Раскрыт ли разбор ориентиров (§15, Р-131).
 *
 * В модуле, а не в хранилище: это положение шторки на одном экране, а не
 * решение человека о том, как приложение должно работать. Держится, пока идёт
 * тренировка, — открывший разбор на первом подходе не закрывает его на
 * каждом следующем.
 */
let разборРаскрыт = false;

/**
 * Ориентиры перед подходом: строка на виду, разбор по нажатию (§15, Р-131).
 *
 * Свёрнутое — то, ради чего сюда смотрят: сколько было в прошлый раз и каков
 * рекорд. Развёрнутое — разбор занятия по подходам, день, собственный вес и
 * совет о запасе, если он есть.
 */
function ориентиры(view, prefill) {
    const { last, best, kind } = view;

    const совет = советЗапаса(view);

    /*
     * Без истории сворачивать нечего, но строка нагрузки нужна и здесь
     * (Р-53, Р-131): она отвечает на набранный довес до записи подхода, и
     * ранний выход отсюда её терял — у нового упражнения её просто не было
     * в разметке.
     */
    if (!last && !best) {
        return ui.html`
            <div class="rec-line rec-empty">${t('Первый раз — ориентиров пока нет')}</div>
            ${loadLine(view, prefill)}
        `;
    }

    /*
     * В свёрнутом виде — первый подход прошлого раза, а не весь разбор.
     *
     * Первый, потому что на него и равняются: к концу упражнения сил меньше,
     * и последний подход всегда слабее — начинать по нему значило бы каждый
     * раз занижать (Р-58).
     */
    const кратко = [
        last ? t('Последний раз {что}', { что: records.describe(last.sets[0], kind) }) : '',
    ...
    return ui.html`
        ...
        <!--
            Нагрузка остаётся снаружи разворота (Р-53, Р-131).

            Строка «с дополнительным весом» пересчитывается прямо во время
            набора: цифру вписывают до записи подхода, и увидеть, во что она
            превратилась, нужно тогда же. За свёрнутой шторкой этого не
            увидишь — а ради этого строка и заведена.
        -->
        ${loadLine(view, prefill)}
    `;
}
 ...
 * следующего раза.
 *
 * Совет правила уехал в разворот ориентиров (Р-131): рядом с ответом ему
 * место по смыслу, но тремя строками в самом тесном месте экрана он выталкивал
 * «Выполнено» под меню. Свёрнутая строка ориентиров о нём объявляет.
 */
function запасБлок() {
...
/**
 * Совет правила прогрессии (§59, Р-131).
 *
 * Стоял под рядом ответов — «подсказка правила рядом с ответом, который её и
 * породил», — и это было верно, пока он не оказался третьей строкой в самом
 * тесном месте приложения. Теперь он живёт в развороте ориентиров, а
 * свёрнутая строка о нём объявляет: спрятанного совета не бывает, бывает
 * непрочитанный.
 *
 * Возвращает разметку или null: по ней же решается, говорить ли «есть совет».
 */
function советЗапаса({ history, kind, exercises }) {
    const занятия = records.sessions(history || []).map((s) => s.sets);
    const итог = reserve.verdict(занятия);

    if (итог.verdict !== 'harder' && итог.verdict !== 'easier') return null;

    const прошлые = kind === 'weight' ? t('вес') : t('сопротивление');

    return итог.verdict === 'harder'
        ? ui.html`
...
function currentCard({ workout, sets, exercises, rows }) {
    ...
    return ui.html`
        <div class="card session-card">
            ...
            <!--
                Ориентиры — в разворот (§15, Р-39, Р-131).

                Свёрнутая строка отвечает на то, ради чего на них смотрят
                перед подходом: сколько было в прошлый раз и каков рекорд. А
                разбор занятия по подходам — «45 повт. — 8 подходов, 40 повт.
                — 2 подхода, 35 повт., 30 повт.» — это уже опись сделанного:
                на двенадцати подходах она занимала две длинные строки, и
                вместе с датой, собственным весом и советом выталкивала
                «Выполнено» под нижнее меню.

                Р-39 свернул эту строку по тому же правилу — ориентир, а не
                опись, — и здесь оно доведено до конца: число на виду, разбор
                по нажатию.

                Совет о запасе уехал сюда же, но молчать не стал: пока он
                есть, свёрнутая строка сама о нём говорит. Спрятанного совета
                не бывает — бывает непрочитанный.
            -->
            <div class="rec-block">
                ${ориентиры(view, prefill)}
            </div>
 ...
 */
/*
 * Разворот ориентиров открывается на месте (§15, Р-131).
 *
