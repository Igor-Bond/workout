    ...
    return ui.html`
        <div class="ex-row" data-id="${exercise.id}">
            <div class="ex-main">
                ...
                ${weighted ? ui.html`
                    <div class="ex-meta">
                        ${t('подходов с дополнительным весом: {сколько}', { сколько: weighted })}
...
actions.on('share-clear', async (el) => {
    ...
    const ok = await dialog.confirm({
        ...
        text: t('Подходов с дополнительным весом у упражнения «{упражнение}»: {сколько}. Он считается тоннажем — как поднятое железо. Убрать его из всех подходов?', {
    ...
    await dialog.alert({
        title: t('Готово'),
        text: t('Дополнительный вес убран. Подходов затронуто: {подходы}, пересчитано тренировок: {тренировки}.', {
