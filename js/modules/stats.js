...
export const stats = {
    ...
    async render() {
        ...
        return ui.html`
            ...
            -->
            <div class="card">
                <div class="card-title">${тоннажЕсть ? t('Объём по группам мышц') : t('Подходы по группам мышц')}</div>
                ${chart.hbars(
                    muscles.map((m) => ({
                        label: t(m.group),
                        value: тоннажЕсть ? Math.round(m.volume) : m.sets,
                        подходы: m.sets
                    })),
                    {
                        format: (v, d) => тоннажЕсть
                            ? `${format.decimal(v, 0)} ${t('кг')} · ${d.подходы}`
                            : `${v}`
                    }
                )}
                <p class="hint">
                    ${тоннажЕсть
                        ? t('Тоннаж группы и число подходов. Тоннаж считает и отягощение, и долю своего веса.')
                        : t('Пока считаются подходы: тоннаж появится, когда будет отмечен вес тела или отягощение.')}
                </p>
    ...
    try {
        ...
        if (e?.reason === 'user') {
            ...
            return dialog.alert({
                ...
                text: t('Номер места или код не подошли. Забыл их — нажмите «Снять с весов» ещё раз и введите заново. Весы показывают и то и другое в настройках пользователя.')
            });
        }
