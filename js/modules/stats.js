...
async function часыБлок() {
    ...
    return ui.html`
        <div class="card">
            ...
            <div class="tiles">
                <!--
                    Сон — «6:10», а не «6 ч 10 мин» (§62, Р-133).
                    
                    Словами он не помещался в плитку и заворачивался в две
                    строки: «6 ч 10» и «МИН» под ним — единственное число на
                    экране, набранное в два этажа, и ряд плиток от него
                    кособочился. Двоеточие между часами и минутами читается
                    без подписи — так пишут время везде, — а секунд у сна нет:
                    их не измеряют, и «7:15:00» было бы неправдой о точности.
                -->
                ${сон !== null ? tile(t('Сон'), `${Math.floor(сон / 3600)}:${String(Math.round((сон % 3600) / 60)).padStart(2, '0')}`) : ''}
                <!--
...
export const stats = {
    ...
    async render() {
        ...
        return ui.html`
            ...
            <!--
                Дорога к кондициям — отсюда (§66).

                Надпись не перечисляет карточки. Перечисляла — «тело, обмен,
                восстановление», — и устарела дважды за один день: сперва
                добавилась форма, потом нагрузка. Список на кнопке обязан
                поспевать за экраном, а он не поспевает никогда.

                Отдельным экраном, а не карточкой здесь: там числа из трёх
                мест сразу и разговор на неделю вперёд, а тут счёт за период.
                Но искать его человеку негде, кроме как рядом с остальными
                числами, — значит ссылка стоит тут.
            -->
            <button class="btn btn-ghost" data-action="nav" data-screen="condition">
                ${t('Кондиции и нагрузка')}
            </button>
...
async function снятьСВесов() {
    ...
    /*
     * Разговор с весами идёт окном, а не полосой наверху (Р-192).
     *
     * Он идёт шагами — ищу, нашёл, встаньте на весы, — и каждый шаг это
     * ответ на вопрос «работает ли вообще». Полоса висела там, куда человек
     * в эту минуту не смотрит: он стоит на весах и смотрит в середину
     * экрана. Шаги приходят в ту же строку окна.
     */
    dialog.waiting({
        title: t('Снимаю с весов'),
        text: t('Ищу весы рядом.')
    });
