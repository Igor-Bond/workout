        ...
        <div class="card">
            ...
            ${свежий ? ui.html`
                <p class="hint ${отстал > 1 ? 'is-bad' : ''}">
                    ${отстал > 1
                        ? t('Самого свежего замера — {день}, и это не осечка приложения: Intervals.icu отвечает, новее у него нет. Смотрите строку ниже — по каждой величине свой срок.', {
                            день: dates.formatDate(свежий)
                        })
                        : t('Самый свежий замер — {день}. По каждой величине срок свой — строкой ниже.', {
                            день: dates.formatDayLabel(свежий, Date.now(), { lower: true })
                        })}
                </p>
            ` : ''}

            <!--
                Свежесть считается по каждой величине отдельно (§62.3, Р-128).
                Их пишет в Intervals.icu не одно и то же: сон и пульс приходят
                ночной выгрузкой, шаги — дневной, и останавливается это
                порознь. Общего «самого свежего замера» тут мало: он скажет,
                что всё в порядке, пока приходит хоть что-нибудь.
            -->
            <div class="plan-rule">
                ...
                <span class="plan-day-rest">
                    ${[
                        ...
                        return свой
                            ? t('{имя} — {n}, по {день}', {
                                имя, n: счёт(поле), день: dates.formatDate(свой)
                            })
                            : t('{имя} — ни одного', { имя });
                    }).join('; ')}
                </span>
