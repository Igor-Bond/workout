        ...
        <div class="card">
            ...
            ${свежий ? ui.html`
                <p class="hint ${отстал > 1 ? 'is-bad' : ''}">
                    ${отстал > 1
                        ? t('Самый свежий замер — {день}. Intervals.icu отвечает исправно, новее у него просто нет: значит молчит то, что пишет туда с часов, а не приложение.', {
                            день: dates.formatDate(свежий)
                        })
                        : t('Самый свежий замер — {день}.', { день: dates.formatDayLabel(свежий, Date.now(), { lower: true }) })}
                </p>
            ` : ''}
