 ...
 *
 * Часы наружу свои данные не отдают, а прямых путей больше нет: Google
 * Fit закрывает REST в конце 2026 года, Health Connect живёт только на
 * Android. Остаётся тот путь, который предлагает приложение часов, — привязка
 ...
 *
 * Настройка идёт через три звена: часы, их приложение, Intervals.icu. Пустой
 * экран в такой цепочке ничего не сообщает — виноватым может быть любое звено, и
...
function привезеноБлок(замеры, занятия) {
    ...
    return ui.html`
        <div class="card">
            ...
            <!--
                Что привезено, а не только сколько (§62.3, Р-137).

                Число отвечает «дошло ли вообще», а спрашивают всегда другое:
                дошла ли та самая тренировка, которую вчера запустили на часах,
                и что от неё осталось. На это счёт не отвечает: он вырастет на
                единицу и от чужой прогулки.

                Пульс, калории и нагрузку приложение уже хранит с прошлой привозки:
                показать их дешевле, чем объяснять, где их искать в чужом
                сервисе. Пять последних, свежие сверху: вопрос «а эта дошла?»
                всегда про последнюю.

                Свёрнуто: на этом экране это справка, а не повод для действия,
                и разворачивают её раз в месяц.
            -->
            ${занятия.rows.length ? ui.html`
                <details class="guide">
                    <summary>${t('Что привезено с часов')}</summary>
                    <div class="guide-body">
                        ${[...занятия.rows]
                            ...
                            .map((з) => ui.html`
                                <div class="plan-day">
                                    <!--
                                        День стоит в строке чисел, а не
                                        отдельным столбцом, как у дней плана.

                                        Столбец забирал бы треть ширины под
                                        одно слово, и на телефоне «пульс 112,
                                        макс 146 · 320 ккал · нагрузка 54»
                                        ложилось в три строки, из которых
                                        последняя — одинокое «54».
                                    -->
                                    <span class="plan-day-body">
                                        ${з.name || з.type || t('занятие')}

                                        <span class="plan-day-rest">${[
                                            dates.formatDayLabel(з.start, Date.now(), { lower: true }),
                                            з.seconds ? t('{n} мин', { n: Math.round(з.seconds / 60) }) : '',
                                            з.avgHr && з.maxHr
                                                ? t('пульс {ср}, макс {макс}', {
                                                    ср: Math.round(з.avgHr), макс: Math.round(з.maxHr)
                                                })
                                                : з.avgHr ? t('пульс {n}', { n: Math.round(з.avgHr) }) : '',
                                            з.calories ? t('{n} ккал', { n: Math.round(з.calories) }) : '',
                                            з.load ? t('нагрузка {n}', { n: Math.round(з.load) }) : ''
                                        ]
                                            .filter(Boolean)
                                            /*
                                             * Внутри величины пробелы
                                             * неразрывные: на телефоне строка
                                             * рвалась по «пульс 112, макс /
                                             * 146», и нагрузка 54 уезжала
                                             * третьей строкой в одиночку.
                                             * Рваться она должна по точкам
                                             * между величинами — там, где и
                                             * проходит граница смысла.
                                             */
                                            .map((часть) => часть.replace(/ /g, ' '))
                                            .join(' · ')}</span>
                                    </span>
                                </div>
                            `)}
    ...
    async render() {
        ...
        return ui.html`
            ...
            <div class="card">
                ...
                <p class="hint">
                    ${цельШагов > 0
                        ? t('Цель — {n} в день. Она видна на главном экране рядом с сегодняшним числом.', {
                            n: `${format.decimal(цельШагов, 0)} ${format.plural(цельШагов, format.WORDS.step)}`
                        })
...
export async function забратьСЧасов({ notSooner = 0, тихо = true } = {}) {
    ...
    if (!тихо) {
         ...
         *
         * Забор идёт двумя запросами в чужой сервис и занимает секунды. До
         * сих пор об этом говорила только надпись на самой кнопке — а она
         * внизу длинного экрана, и человек, нажав, видел ровно то же, что и
         * до нажатия.
         */
        dialog.waiting({
            title: t('Забираю с часов'),
            text: t('Спрашиваю у Intervals.icu замеры и занятия за месяц. Обычно ответ приходит за несколько секунд.')
        });
    }
    ...
    try {
        ...
        if (занятия.status === 'fulfilled') {
             ...
             *
             * Пустой список значит одно из трёх: занятий правда нет, часы их
