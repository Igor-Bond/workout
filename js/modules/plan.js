...
/**
 * Типы тренировок. Ключи, а не подписи: в записанную тренировку уходит
 * подпись на языке того, кто её проводил (§53).
 *
 * Экспортируются ради истории: тип правится и оттуда (Р-50), а два списка
 * одних и тех же слов разошлись бы при первом же добавлении.
 */
export const WORKOUT_TYPES = ['Силовая', 'Зарядка', INTERVAL, 'Кардио', 'Растяжка', 'Дома без инвентаря'];
...
async function build(params) {
    ...
    /*
     * Повтор тренировки (§9): за основу берётся фактически выполненное, а
     * не то, что планировалось.
     *
     * Без идентификатора повторяется последняя — это кнопка «Повторить
     * прошлую». С идентификатором — названная: плашки частых составов на
     * главном ведут сюда же (§29.1), и повторять они должны именно свою.
     */
    if (what === 'repeat') {
        const last = id
            ? await dbService.getWorkout(id)
            : (await dbService.listWorkouts({ limit: 1 }))[0];
...
export const plan = {
    ...
    /*
     * Полоса «Убрано … · Вернуть» не переживает ухода с экрана (Р-119).
     *
     * Черновик уход переживает — это задумано, — а предложение вернуть строку
     * нет: висящее на главном или в истории, оно звало бы назад в план,
     * которого человек в эту минуту не собирает. Сам черновик при этом цел, и
     * убранное вернётся правкой вручную.
     */
    leave() {
        убрано = null;
        app.hideBanner('plan-undo');
    }
};
...
function reset() {
    ...
    // Возвращать некуда: черновика больше нет
    убрано = null;
    app.hideBanner('plan-undo');
}
...
/**
 * Последняя убранная строка — чтобы её можно было вернуть (Р-119).
 *
 * Одна, а не стопка: «вернуть» нужно сразу после промаха, и предлагать
 * откат на пять шагов назад значило бы обещать историю правок, которой у
 * экрана нет.
 */
let убрано = null;

/*
 * Убранное возвращается, а не восстанавливается по памяти (§10, Р-119).
 *
 * Вместе со строкой уходят подходы, повторения и вес, вписанные руками
 * минуту назад. Спрашивать перед каждым «убрать» нельзя — состав собирают и
 * разбирают, это обычное движение, — а вот путь назад обязателен.
 */
actions.on('plan-remove', (el) => {
    const index = Number(el.dataset.index);
    const [строка] = draft.items.splice(index, 1);

    убрано = { index, строка };

    // Полоса пересоздаётся: showBanner молча уходит, если такая уже висит, а
    // назвать она должна последнее убранное, а не первое
    app.hideBanner('plan-undo');
    app.showBanner('plan-undo', String(ui.html`
        <span>${t('Убрано: {что}', { что: строка.name })}</span>
        <button class="banner-btn" data-action="plan-undo">${t('Вернуть')}</button>
    `));

    app.render();
});

actions.on('plan-undo', () => {
    if (!убрано || !draft) return app.hideBanner('plan-undo');

    // На прежнее место, а не в конец: строка стояла там не случайно — порядок
    // упражнений в тренировке и есть то, что на этом экране настраивают
    draft.items.splice(Math.min(убрано.index, draft.items.length), 0, убрано.строка);
