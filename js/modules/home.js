...
function startBlock(last, templates, suggestion, names, due, frequent, очередь, записи, ежедневное, поПлану) {
    ...
    /*
     * Первый убран из плашек, только если он правда стоит карточкой (Р-72).
     *
     * Карточку занимает план, когда сегодня есть занятие по нему, — и тогда
     * первый из очереди не стоит нигде: ни карточкой, ни плашкой. Состав,
     * до которого дольше всего не доходили, молча исчезал с экрана ровно в
     * те дни, когда план и так велит другое.
     */
    const наКарточке = !поПлану && первое;
    const хвост = наКарточке ? очередь.slice(1) : очередь;
