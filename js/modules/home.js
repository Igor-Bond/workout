...
function startBlock(last, templates, suggestion, names, due, frequent, очередь, записи, ежедневное, поПлану, план) {
    ...
    const ближайшие = активен
        ? planCore.expand(план, { from: Date.now() + DAY, days: 14 })
            .filter((д) => д.session)
            .slice(0, 3)
        : [];
        ...
        <div class="section">
            ...
            ` : первое ? ui.html`
                ...
                <button class="repeat-card is-queue" data-action="home-like" data-id="${первое.workoutId}">
                    <span class="rep-label">${активен ? t('Если хочется добавить') : t('На очереди')}</span>
