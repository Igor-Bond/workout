...
function startBlock(last, templates, suggestion, names, due, frequent, очередь, записи) {
     ...
     * делали вовремя.
     *
     * Подпись — опоздание, и это третья её редакция. Сначала стоял множитель
     * повторов, потом дни с прошлого раза — и оба раза владелец справедливо
     * спрашивал, почему порядок им противоречит. Сортировка идёт по
     * опозданию, значит опоздание и должно стоять на плашке: подпись обязана
     * называть то, по чему выстроен ряд, иначе ряд читается как поломка.
     *
     * Заодно она говорит то, чего дни с прошлого раза не говорили вовсе:
     * пора уже или ещё рано. «+3 дн» — опоздал, «через 4 дн» — срок не
     * вышел, «пора» — ровно сегодня.
     *
    ...
    const chips = хвост.length
        ? хвост.map((f) => ui.html`
            <button class="chip" data-action="home-like" data-id="${f.workoutId}">
                ...
                <span class="chip-count">${опоздание(f)}</span>
            </button>
        `)
        : frequent.length
