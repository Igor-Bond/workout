...
/**
 * Через сколько показывать «Загрузка…» при переходе (Р-59, Р-76).
 *
 * Четыреста миллисекунд. Двести оказалось мало: на телефоне переход, который
 * на разработке занимает тридцать миллисекунд, длится дольше порога — и
 * заглушка успевает мелькнуть и исчезнуть. Мелькнувшая заглушка хуже её
 * отсутствия: человек видит не переход, а вспышку, и считает, что приложение
 * подгружает несколько экранов подряд.
 *
 * Порог заодно и есть определение «долго»: всё, что быстрее, человек читает
 * как мгновенное, и объявлять там загрузку не о чем.
 */
const PLACEHOLDER_DELAY = 400;
...
export const app = {
    ...
    async render() {
        ...
        try {
            ...
            /*
             * Одинаковую разметку не подставляем заново (Р-76).
             *
             * Перерисовка случается по многим поводам — приехал обмен,
             * сменился полноэкранный режим, закрылось окно, — и часть из них
             * ничего не меняет на экране. Замена innerHTML на точно такой же
             * всё равно стоит браузеру перерисовки: на телефоне это видно
             * вспышкой, а человеку выглядит как «подгружается ещё один
             * экран».
             */
            if (host.innerHTML !== разметка) host.innerHTML = разметка;
        ...
        /*
         * Прокрутка возвращается после вставки разметки: до неё высота
         * страницы ещё прежняя, и браузер обрежет значение по ней
         */
        полоса.scrollTop = scroll;

        /*
         * Клавиши прокручивают только то, что в фокусе (Р-78).
         *
         * Страница не двигается вовсе — двигается содержимое каркаса, — и
         * пока фокус на теле документа, Page Down и стрелки не делают ничего.
         * Отдаём фокус области прокрутки, но только если он ни на чём: у
         * человека, набирающего заметку, отбирать его нельзя.
         */
        if (document.activeElement === document.body) полоса.focus({ preventScroll: true });
