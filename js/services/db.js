...
/**
 * Завёл ли упражнение человек, а не базовый список.
 *
 * Признак — идентификатор: у базовых он выведен из названия и начинается с
 * `base-`, у самодельных это UUID. Нужен при сведении двойников, чтобы
 * решить, чьи свойства оставить (§5.1).
 */
const userMade = (exercise) => !String(exercise?.id || '').startsWith('base-');
...
export const dbService = {
    ...
    /**
     * Сколько живых подходов ссылается на упражнение — от этого зависит
     * удаление (§5, Р-122).
     *
     * Живых, а не всех: удаление подхода помечает запись, а не стирает её —
     * иначе удалённое на одном устройстве вернулось бы с другого при первом
     * же обмене. Счёт же брал все строки подряд, вместе с надгробиями.
     *
     * Отсюда две беды сразу. В справочнике у упражнения стояло «12 подходов»,
     * когда его история стёрта до единого подхода. И удалить его было нельзя
     * уже никогда: «встречается в истории» — хотя не встречается. Стереть
     * заведённое по ошибке упражнение можно было только до первого подхода,
     * а после — ни до, ни после удаления этих подходов.
     */
    async countSetsOfExercise(exerciseId) {
        const все = await db.sets.where('exerciseId').equals(exerciseId).toArray();

        return все.filter(alive).length;
    },
    ...
    async mergeExercises(sourceId, targetId) {
        ...
        await db.transaction('rw', db.exercises, db.sets, db.workouts, db.templates, async () => {
            ...
            /*
             * Свойства победителя дополняются свойствами проигравшего (§5.1).
             *
             * Победителем становится строго меньший идентификатор, и раньше
             * он приносил свою группу и вид, а чужие пропадали. У базовых
             * упражнений идентификатор начинается с `base-`, у самодельных
             * это случайный UUID, который начинается со знака от 0 до f:
             * меньше `b` — побеждает самодельное, больше — базовое. То есть
             * свойства выбирала монетка.
             *
             * Пока группа была нужна лишь для поиска в справочнике, цена
             * была косметической. С появлением отдыха мышц (§29.1) группа
             * двигает то, что приложение предлагает делать.
             *
             * Правило: непустое побеждает пустое, а при двух разных
             * непустых — пользовательское, то есть не `base-`. Человек,
             * проставивший группу руками, знает про своё упражнение больше,
             * чем список из коробки. Оба признака не зависят от порядка
             * обмена, поэтому все устройства приходят к одному результату.
             */
            const patch = {};

            /*
             * Кроме группы и описания переезжает всё, что человек сказал об
             * упражнении делом (Р-100): длительность отдыха (Р-46),
             * постоянный довес резинки (Р-86), единица времени (Р-87) и своя
             * доля собственного веса (§15.2).
             *
             * Владелец объединил «Бицепс резинка» со «Сгибанием рук с
             * резинкой» — и на следующей зарядке пауза у сгибаний оказалась
             * чужой: помнилась она за исчезнувшей записью, а победителю не
             * досталась. Со стороны это выглядит как «упражнение стало
             * новым», и доверия к объединению не прибавляет.
             */
            for (const field of ['group', 'howTo', 'restSeconds', 'defaultWeight', 'timeUnit', 'bodyShare']) {
                const свой = target[field];
                const чужой = source[field];

                if (чужой && (!свой || (свой !== чужой && !userMade(target) && userMade(source)))) {
                    patch[field] = чужой;
                }
            }

            // Вид не бывает пустым — у него есть умолчание, поэтому здесь
            // решает только происхождение записи
            if (source.kind && source.kind !== target.kind && !userMade(target) && userMade(source)) {
                patch.kind = source.kind;
            }

            if (Object.keys(patch).length) {
                await db.exercises.update(targetId, { ...patch, updatedAt: now });
            }
    ...
    /**
     * Свёртка давних дней еды в одно число (§68, Р-165).
     *
     * Записей о еде больше, чем всех остальных вместе: три-пять в день против
     * трёх тренировок в неделю. Год даёт около полутора тысяч документов, и
     * растёт это без конца, тогда как смысл у старого дня остаётся ровно
     * один — сколько всего съедено. Отдельные приёмы пищи годичной давности
     * не читает никто.
     *
     * Свёртка возможна потому же, почему тренировка уезжает в облако одним
     * документом вместе с подходами: **закрытый день больше не меняется**. В
     * сегодняшний дописывают весь день, и сворачивать его нельзя — два
     * устройства переписали бы друг другу сумму. Год назад дописывать нечего.
     *
     * Остаётся самая ранняя запись дня, а не новая: у неё уже есть номер,
     * который знают оба устройства. Свёртка на двух устройствах порознь даёт
     * тот же номер и ту же сумму — то есть она повторима, и обмен не создаст
     * двойников. Прочие записи дня уходят мягким удалением, как всё
     * остальное (§36).
     *
     * Заметки склеиваются, а не выбрасываются (Р-175). Сперва они пропадали,
     * и я назвал это ценой: «у суммы пяти приёмов пищи нет одной заметки, а
     * склеенная из пяти — мусор». Это было неверно. Через год смотрят на
     * дневной итог, и единственный вопрос к нему — «почему столько»;
     * «завтрак; обед; пицца» отвечает на него, а пустота не отвечает ни на
     * что. Весит такая строка десятки байт при экономии в сотни.
     */
    async foldIntake({ before }) {
        if (!Number.isFinite(before) || before <= 0) return { days: 0, removed: 0 };

        const rows = (await db.intake.where('at').below(before).toArray()).filter(alive);

        const поДням = new Map();

        for (const r of rows) {
            const день = dates.startOfDay(r.at);
            поДням.set(день, [...(поДням.get(день) || []), r]);
        }

        const now = Date.now();

        /*
         * Правки собираются и кладутся одним заходом.
         *
         * Первая свёртка после года ведения дневника трогает около трёхсот
         * шестидесяти дней и полутора тысяч записей. Поштучная правка — это
         * полторы тысячи отдельных обращений к хранилищу при запуске
         * приложения, то есть заметная задержка на телефоне ровно там, где её
         * никто не ждёт.
         */
        const кпереписи = [];
        ...
        for (const записи of поДням.values()) {
            if (записи.length < 2) continue;

            // Порядок один на всех устройствах: по времени, при равенстве — по номеру
            const порядок = [...записи].sort((a, b) => a.at - b.at || (a.id < b.id ? -1 : 1));

            const [главная, ...прочие] = порядок;
            const сумма = порядок.reduce((s, r) => s + (Number(r.kcal) || 0), 0);

            /*
             * Порядок заметок — порядок дня, повторы убираются: «обед» дважды
             * не говорит вдвое больше. Длина ограничена, потому что заметка
             * это подпись, а не дневник: за пределом строка обрывается
             * многоточием, и видно, что было не всё.
             */
            const заметки = [...new Set(
                порядок.map((r) => String(r.note || '').trim()).filter(Boolean)
            )].join('; ');

            кпереписи.push({
                ...главная,
                kcal: Math.round(сумма),
                note: заметки.length > 200 ? `${заметки.slice(0, 199)}…` : заметки,
                updatedAt: now
            });

            for (const r of прочие) {
                кпереписи.push({ ...r, deletedAt: now, updatedAt: now });
            }

            дней += 1;
        }

        if (кпереписи.length === 0) return { days: 0, removed: 0 };

        await db.intake.bulkPut(кпереписи);

        return { days: дней, removed: кпереписи.length - дней };
    },
