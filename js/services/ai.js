...
/**
 * Список моделей, привезённый от Google, — с отметкой времени (§60.3, Р-198).
 *
 * Хранится затем, что показывается он списком при каждом открытии экрана, а
 * ходить за ним в сеть при каждом открытии незачем: состав меняется раз в
 * месяцы, а экран открывают каждый день.
 *
 * Не синхронизируется: он выведен из ключа, а ключ у каждого устройства свой.
 */
export const MODELS_SETTING = 'aiModels';

/**
 * Через сколько список считается несвежим.
 *
 * Сутки. Реже — и человек, у которого Google выключил модель утром, узнает об
 * этом через неделю; чаще — и приложение ходит в сеть ради ответа, который не
 * меняется месяцами.
 */
export const MODELS_TTL = 86400000;
