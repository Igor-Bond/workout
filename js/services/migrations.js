...
/**
 * Где в базовом списке лежат интервальные упражнения (§50).
 *
 * Список растёт только с конца (иначе поедет доливка), и блоки в нём идут по
 * порядку появления: зал и своё тело, интервальные, домашний набор с
 * резинкой, гиря. Границы интервального блока названы здесь, потому что к
 * нему есть требование, которого нет у остальных: снаряда в этих упражнениях
 * быть не должно — вес между двадцатисекундными отрезками вводить некогда.
 */
export const INTERVAL_BLOCK = [31, 45];
...
export const BASE_EXERCISES = [
    ...
    /*
     * Дальше — домашний набор: резинка, турник, брусья (§5).
     *
     * Дописано в конец и только в конец. Доливка тем, у кого база уже
     * создана, берёт хвост списка по числу уже доставленных: вставка в
     * середину сдвинула бы отсчёт, и человеку приехало бы не то.
     *
     * Первые сорок шесть упражнений — зал и своё тело, а дома с резинкой на
     * весь верх приходилось три упражнения на руки и два на плечи. Тому, кто
     * тренируется дома, справочник предлагал заводить всё самому.
     */
    { name: 'Тяга резинки к поясу',     kind: 'reps',     group: 'Спина' },
    { name: 'Тяга резинки сверху',      kind: 'reps',     group: 'Спина' },
    { name: 'Разведение резинки',       kind: 'reps',     group: 'Спина' },
    { name: 'Жим резинки от груди',     kind: 'reps',     group: 'Грудь' },
    { name: 'Сведение рук с резинкой',  kind: 'reps',     group: 'Грудь' },
    { name: 'Жим резинки вверх',        kind: 'reps',     group: 'Плечи' },
    { name: 'Махи с резинкой в стороны', kind: 'reps',    group: 'Плечи' },
    { name: 'Подъём резинки перед собой', kind: 'reps',   group: 'Плечи' },
    { name: 'Сгибание рук с резинкой',  kind: 'reps',     group: 'Руки' },
    { name: 'Разгибание рук с резинкой', kind: 'reps',    group: 'Руки' },
    { name: 'Приседания с резинкой',    kind: 'reps',     group: 'Ноги' },
    { name: 'Отведение ноги с резинкой', kind: 'reps',    group: 'Ноги' },
    { name: 'Австралийские подтягивания', kind: 'reps',   group: 'Спина' },
    { name: 'Подтягивания обратным хватом', kind: 'reps', group: 'Руки' },
    { name: 'Отжимания узким хватом',   kind: 'reps',     group: 'Руки' },
    { name: 'Обратные отжимания',       kind: 'reps',     group: 'Руки' },
    { name: 'Отжимания в наклоне',      kind: 'reps',     group: 'Плечи' },
    { name: 'Вис на турнике',           kind: 'time',     group: 'Спина' },

    /*
     * Гиря — дописано в конец, как и всё остальное (§5).
     *
     * Дома с резинкой не получить двух вещей: работы задней цепи с высоким
     * пульсом и настоящего веса в руке. Гиря даёт обе, стоит дешевле любого
     * набора гантелей и занимает угол. Шесть упражнений — это весь домашний
     * гиревой минимум: мах, присед, тяга, жим, становая и подъём.
     */
    { name: 'Махи гирей',               kind: 'weight',   group: 'Ноги' },
    { name: 'Гоблет-приседания',        kind: 'weight',   group: 'Ноги' },
    { name: 'Становая тяга с гирей',    kind: 'weight',   group: 'Спина' },
    { name: 'Тяга гири в наклоне',      kind: 'weight',   group: 'Спина' },
    { name: 'Жим гири стоя',            kind: 'weight',   group: 'Плечи' },
    { name: 'Турецкий подъём',          kind: 'weight',   group: 'Всё тело' },

    /*
     * Дописано в конец следом за гирей (§5).
     *
     * Отжимания Тайсона — обычное домашнее упражнение, которого в списке не
     * хватало: владелец завёл его руками, а с ним и группу «Трицепс», которой
     * в приложении нет. Группа здесь «Грудь», как у прочих отжиманий: акцент
     * тот же, а разная группа у одного движения ломает и отдых мышц, и объём
     * по группам (§29.1, §26.1).
     *
     * Баскетбол — игра, а не упражнение, и всё же место ему здесь: для того,
     * кто играет, это главная нагрузка недели. Группа «Кардио»: ноги игра
     * утомляет, но не тренирует, и пометить её ногами значило бы сказать
     * приложению, что день ног закрыт.
     */
    { name: 'Отжимания Тайсона',        kind: 'reps',     group: 'Грудь' },
    { name: 'Баскетбол',                kind: 'time',     group: 'Кардио' }
];
