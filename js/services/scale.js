...
/** Пауза, после которой очередь замеров считается кончившейся. */
const ТИШИНА = 4000;
...
export const scale = {
    ...
    /**
     * Свести весы с составами в замеры.
     *
     * Приходят они двумя потоками — по одной записи на каждое взвешивание, —
     * и связать их можно только порядком: состав тела метки времени не несёт
     * вовсе, а вес несёт. Порядок надёжен, потому что весы отдают накопленное
     * подряд, парами и в хронологии.
     *
     * Состава может не быть совсем: человек встал в носках, и ток через тело
     * не прошёл. Вес при этом приезжает, и терять его незачем.
     */
    pair(веса = [], составы = []) {
        return веса.map((вес, i) => ({ weight: вес, body: составы[i] || null }));
    },

    /**
     * Снять замеры с весов.
     *
     * Возвращает список { weight, body } — по одному на взвешивание. Не один
     * замер, а все: весы хранят сделанное в памяти места и отдают накопленное
     * сразу, как только узнают человека. Значит, вставать на весы с телефоном
     * в руке необязательно — можно взвеситься утром, а спросить весы вечером,
     * и приедет утреннее.
     *
     * Место и код обязательны: без них весы молчат, а заводить своё место
     * нельзя — на экране весов оно не показывается, и выбрать его человек не
     * сможет.
     *
    ...
    async read({ index, code, onStatus = () => {} } = {}) {
        ...
        try {
            ...
            /*
             * Ждём не первую запись, а тишину после последней.
             *
             * Накопленное весы отдают очередью, и остановиться на первой паре
             * значило бы потерять все взвешивания, кроме самого раннего.
             * Сколько их будет, заранее не знает никто, поэтому признаком
             * конца служит пауза: перестали присылать — значит, всё.
             */
            let готово;
            ...
            const приехало = () => {
                clearTimeout(тишина);
                тишина = setTimeout(() => { if (веса.length) готово(); }, ТИШИНА);
            };

            const измерение = await служба.getCharacteristic(ИЗМЕРЕНИЕ);

            измерение.addEventListener('characteristicvaluechanged', (e) => {
                веса.push(scale.parseWeight(e.target.value));
                onStatus(`Приехало взвешиваний: ${веса.length}…`);
                приехало();
            });
            ...
            try {
                ...
                сИзм.addEventListener('characteristicvaluechanged', (e) => {
                    составы.push(scale.parseBody(e.target.value));
                    приехало();
                });

                await сИзм.startNotifications();
            } catch (e) {
                /* без состава тела, но с весом */
            }
            ...
            /*
             * Отказ опознать помечается особо (Р-110).
             *
             * Это единственная беда разговора, которую человек может починить
             * сам: номер места и код он списывал с экрана весов на глаз, и
             * ошибиться в них — обычное дело. Всё остальное — сон весов,
             * разрыв связи, отказ браузера — от него не зависит, и звать его
            ...
            onStatus('Жду замеры. Если новых нет — встаньте на весы.');
            ...
            if (!веса.length) {
                throw new Error('Весы ничего не прислали. Новых взвешиваний у них нет, а свежее они отдают после того, как на них выбрано ваше место.');
            }

            return scale.pair(веса, составы);
        } finally {
            try {
