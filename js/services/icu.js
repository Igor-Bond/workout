 ...
 *
 * Часы пишут сон и пульс покоя в облако своего приложения — Zepp у Amazfit,
 * HUAWEI Health у Huawei, — и наружу оно их само не отдаёт. Прямого пути
 * оттуда к приложению нет: Google Fit закрывает свой REST в конце 2026 года,
 * Health Connect живёт только на Android. Остаётся тот путь, который
 * предлагают сами эти приложения, — привязка стороннего сервиса, и
 * единственный в списке, у кого есть открытый ключ на чтение, это
 * Intervals.icu.
 *
 * Значит, цепочка такая: часы → приложение часов → Intervals.icu →
 * приложение. Звеньев многовато, но каждое из них человек настраивает один
 * раз, а альтернатива — вписывать сон руками, чего никто делать не станет.
 *
...
export const icu = {
     ...
     *
     * Пустой список занятий значит одно из трёх: занятий правда нет,
     * приложение часов их не отдаёт, или наше не узнало полей. Различить это со стороны
        ...
        return data
            .map((row) => {
                ...
                return {
                    ...
                    /*
                     * Форма и усталость считает сам Intervals.icu (§66).
                     *
                     * CTL — нагрузка, накопленная за шесть недель, ATL — за
                     * последнюю; их разность и есть «форма». Считать это
                     * самим было бы нечем: нужен поминутный пульс, а наружу
                     * отдаётся только средний за занятие. Зато сервис уже
                     * посчитал — и отдаёт в том же ответе, что и сон.
                     *
                     * Придут они не у всех: у Intervals.icu должно быть из
                     * чего считать нагрузку, то есть пульс или мощность в
                     * занятиях. Нет — останутся пустыми, и карточка формы
                     * просто не появится.
                     */
                    ctl: число(row?.ctl),
                    atl: число(row?.atl),

                    // Оценку сна считает само приложение часов, и там же она
                    // видна: мы её не толкуем, а передаём
...
/**
 * Дневная цель по шагам (§62.5, Р-89).
 *
 * Ходьба — та часть работы над собой, которую тренировка не покрывает:
 * четыре занятия в неделю по часу это четыре часа, а сутки идут все семь
 * дней. Цель человек ставит сам: приложение не знает, сколько ему нужно, и
 * готовых десяти тысяч из рекламы шагомеров тут не будет.
 */
export const ICU_STEPS_GOAL = 'stepsGoal';
